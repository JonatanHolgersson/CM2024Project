...
//--- ENDAST RUBRIK TA BORT SENARE ---------------------------------------------------------------------------------------------------
	printf("tid_ms;ax;ay;az;gx;gy;gz;a_total\n\n");
//------------------------------------------------------------------------------------------------------------------------------------
	...
	while (1) {
		...
		sprintf(out_str, "%d;%f;%f;%f;%f;%f;%f;%f", cnt * 2000,	ax, ay, az, sensor_value_to_double(&gyro_x_out), sensor_value_to_double(&gyro_y_out), sensor_value_to_double(&gyro_z_out), a_total);
